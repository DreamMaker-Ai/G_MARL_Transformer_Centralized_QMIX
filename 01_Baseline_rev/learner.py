...
class Learner:
    ...
    def define_network(self):
        ...
        # q, target_q:(1,n,action_dim),
        # masked_features, target_masked_features:(1,n,hidden_dim)
        q, _, masked_features = self.q_network(padded_obs, mask, training=True)
        target_q, _, target_masked_features = \
            self.target_q_network(padded_obs, mask, training=False)

        # build hyper_mixing_network & target_hyper_mixing_network
        self.hyper_mixing_network(masked_features, q[:, :, 0])
        self.target_hyper_mixing_network(target_masked_features, target_q[:, :, 0])
    ...
    def update_network(self, minibatchs):
        ...
        for (indices, correction_weights, experiences) in minibatchs:
            ...
            # Target valueの計算
            next_q_logits, _, next_masked_features = \
                self.target_q_network(next_states_for_q, masks, training=False)
            # (32,n,5), # (32,n,256)
            ...
            next_maxQ = self.target_hyper_mixing_network(next_masked_features, next_maxQ)  # (32,1)
            ...
            with tf.GradientTape() as tape:
                q_logits, _, masked_features = \
                    self.q_network(states, masks, training=True)  # (32,n,5)
                ...
                Q = self.hyper_mixing_network(masked_features, Q)  # (32,1)
