...
class MarlTransformerModel(tf.keras.models.Model):
    ...
    =================================================================
     input_1 (InputLayer)        [(None, 20, 15, 15, 6)]   0

     cnn_model (CNNModel)        (None, 20, 256)           258944

     dropout (Dropout)           (None, 20, 256)           0

     multi_head_attention_model   ((None, 20, 256),        526080
     (MultiHeadAttentionModel)    (None, 2, 20, 20))

     multi_head_attention_model_  ((None, 20, 256),        526080
     1 (MultiHeadAttentionModel)   (None, 2, 20, 20))

     q_logits_model (QLogitsMode  (None, 20, 5)            395525
     l)
    ...
    def call(self, x, mask, training=True):
        ...
        """ CNN layer """
        masked_features = self.cnn(x, mask)  # (1,n,hidden_dim)

        """ Dropout layer """
        features_cnn = self.dropout(masked_features, training=training)
        ...
        return q_logits, [score1, score2], masked_features
    ...
    def build_graph(self, mask):
        ...
        masked_features = self.cnn(x, mask)

        features_cnn = self.dropout(masked_features, training=True)
        ...
        model = tf.keras.models.Model(
            inputs=[x],
            outputs=[q_logits, [score1, score2], masked_features],
...
def main():
    ...
    q_logits, scores, masked_features = marl_transformer(padded_obs, mask, training=True)
