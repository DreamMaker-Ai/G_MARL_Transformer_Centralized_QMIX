...
class Tester:
    ...
    def reset_states(self, observations):
        ...
        q, _ = self.policy(self.padded_states, self.mask, training=False)
        global_state = np.random.rand(1, self.obs_shape[0], self.obs_shape[1],
                                      self.env.config.hyper_mixing_obs_channels *
                                      self.env.config.hyper_mixing_n_frames)  # (1,15,15,8)
        self.hyper_mixing(global_state, q[:, :, 0])  # build
    ...
    def test_play(self, current_weights, epsilon, current_global_weights):
        ...
        for _ in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                next_obserations, rewards, dones, infos, reward, done, global_observation = \
