...
def compute_red_pos_maps(env):
    ...
    for red in env.reds:
        if red.alive:
            sin_map[red.pos[0], red.pos[1]] += (np.sin(i) + 1.) / 2.
            cos_map[red.pos[0], red.pos[1]] += (np.cos(i) + 1.) / 2.

            i += 1

    normalized_sin_map = 0.5 * 2.0 / np.pi * np.arctan(sin_map)
    normalized_cos_map = 0.5 * 2.0 / np.pi * np.arctan(cos_map)
