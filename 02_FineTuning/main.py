...
def main(num_actors=8):
    ...
    current_weights, current_global_weights = ray.get(learner.define_network.remote())
    current_weights = ray.put(current_weights)
    current_global_weights = ray.put(current_global_weights)
    ...
    wip_actors = [actor.rollout.remote(current_weights, current_global_weights) for actor in actors]
    ...
    for _ in range(config.actor_rollouts_before_train):
        ...
        wip_actors.extend([actors[pid].rollout.remote(current_weights, current_global_weights)])
        # 新しいobject refsを追加
    ...
    # test実施
    wip_tester = tester.test_play.remote(current_weights=current_weights, epsilon=0.0,
    ...
    while update_cycles <= config.num_update_cycles:
        ...
        if finished_learner:
            ...
            current_weights, indices, td_errors, mean_loss, current_global_weights = \
        ...
        if finished_tester:
            ...
            wip_tester = tester.test_play.remote(current_weights=current_weights,
                                                 epsilon=0.0,
                                                 current_global_weights=current_global_weights)
