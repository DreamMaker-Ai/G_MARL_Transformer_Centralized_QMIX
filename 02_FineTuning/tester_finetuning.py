...
class Tester:
    ...
    def reset_states(self, observations):
        ...
        # Build policy & mixing network
        q, _, masked_features = self.policy(self.padded_states, self.mask, training=False)
    ...
    def test_play(self, current_weights, epsilon, current_global_weights):
        # 重みを更新
        ...
        for _ in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                q_logits, scores, _ = self.policy(self.padded_states, self.mask, training=False)
        ...
        if result['num_red_win'] >= self.num_max_win:
            ...
            save_name = '/best_global_model/'
            self.hyper_mixing.save_weights(str(save_dir) + save_name)
...
def main():
    ...
    load_name = '/Scenario_3_finetuning/models/best_model/'
    global_load_name = '/Scenario_3_finetuning/models/best_global_model/'
