...
from qmix_models_rev import HyperMixingNetwork
...
class Actor:
    def __init__(self, pid, epsilon):
        ...
        q, _, masked_features = self.policy(self.padded_states, self.mask, training=True)  # build
        # q:(b,n,action_dim), masked_features:(b,n,hidden_dim)

        self.hyper_mixing(masked_features, q[:, :, 0])  # build
    ...
    def rollout(self, current_weights, current_global_weights):
        ...
        for _ in range(self.env.config.actor_rollout_steps):
            ...
            q_logits, _, _ = self.policy(self.padded_states, self.mask, training=False)
        ...
        if self.env.config.prioritized_replay:
            ...
            """ compute target """
            # next_q_values:(b,n,action_dim), next_masked_features:(b,n,hidden_dim)
            next_q_values, _, next_masked_features = \
                self.policy(next_states_for_q, mask, training=False)
            ...
            next_maxQ = self.hyper_mixing(next_masked_features, next_maxQ)  # (b,1)
            next_maxQ = tf.squeeze(next_maxQ, axis=-1)  # (b,)

            TQ = rewards + self.gamma * (1 - dones) * next_maxQ  # (b,)

            """ compute Q """
            # q_values:(b,n,action_dim), masked_features:(b,n,hidden_dim)
            q_values, _, masked_features = self.policy(states, mask,
                                                       training=False)  # (b,n,action_dim)
            ...
            Q = self.hyper_mixing(masked_features, Q)  # (b,1)
