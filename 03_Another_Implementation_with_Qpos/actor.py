...
class Actor:
    def __init__(self, pid, epsilon):
        ...
        # Make a q_network & HyperMixing Network
        self.policy = MarlTransformerModel(config=self.env.config)
        self.hyper_mixing = HyperMixingNetwork(hidden_dim=self.env.config.hyper_mixing_hidden,
        ...
        self.hyper_mixing_n_frames = self.env.config.hyper_mixing_n_frames
        ...
        q, _ = self.policy(self.padded_states, self.mask, training=True)  # build
        self.hyper_mixing(self.global_state, q[:, :, 0])  # build
    ...
    def reset_states(self, observations, global_observation):
        ...
        self.global_frames = deque([global_observation] * self.hyper_mixing_n_frames,
                                   maxlen=self.hyper_mixing_n_frames)

        global_state = np.concatenate(self.global_frames, axis=2).astype(np.float32)
        # (g,g,mixing_ch * mixing_n_frames)=(15,15,4*2)

        self.global_state = np.expand_dims(global_state, axis=0)
        # (1,g,g,mixing_ch * mixing_n_frames)=(1,15,15,4*2)
    ...
    def rollout(self, current_weights, current_global_weights):
        """
                ...
                transitions=[transition,...], len=self.env.config.actor_rollout_steps=10
                    transition =
                        (
                            ...
                            global_state,  # (1,g,g,mixing_ch*mixing_n_frames)
                            next_global_state,  # (1,g,g,mixing_ch*mixing_n_frames)
                        )
        """
        # 重みを更新
        self.policy.set_weights(weights=current_weights)
        self.hyper_mixing.set_weights(weights=current_global_weights)
        ...
        for _ in range(self.env.config.actor_rollout_steps):
            ...
            # (g,g,mixing_ch*mixing_n_frames)=(15,15,8)

            next_global_state = np.expand_dims(next_global_state, axis=0)
            # (1,g,g,mixing_ch*mixing_n_frames)=(1,15,15,8)
            ...
            transition = (
                ...
                next_padded_states_for_q,  # (1,n,g,g,ch*n_frames)
                alive_agents_ids,  # (1,a), a=num_alive_agent, object
                self.global_state,  # (1,g,g,mixing_ch*mixing_n_frames)
                next_global_state,  # (1,g,g,mixing_ch*mixing_n_frames)
            )
        ...
        if self.env.config.prioritized_replay:
            ...
            next_states = \
                np.vstack([transition[3] for transition in self.buffer])  # (b,n,g,g,ch*n_frames)
            dones = np.array([transition[4] for transition in self.buffer])  # (b,)
            mask = np.vstack([transition[5] for transition in self.buffer])  # (b,n)
            next_states_for_q = \
                np.vstack([transition[7] for transition in self.buffer])  # (b,n,g,g,ch*n_frames)

            global_states = np.vstack([transition[9] for transition in self.buffer])
            # (b,g,g,mixing_ch*mixing_n_frames)
            next_global_states = np.vstack([transition[10] for transition in self.buffer])
            # (b,g,g,mixing_ch*mixing_n_frames)
            ...
            next_maxQ = self.hyper_mixing(next_global_states, next_maxQ)  # (b,1)
            next_maxQ = tf.squeeze(next_maxQ, axis=-1)  # (b,)
            ...
            Q = self.hyper_mixing(global_states, Q)  # (b,1)
            Q = tf.squeeze(Q, axis=-1)  # (b,)
